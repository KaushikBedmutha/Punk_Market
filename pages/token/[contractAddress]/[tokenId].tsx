...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  // const { contract: bidContract } = useContract(MARKETPLACE_ADDRESS, 'marketplace');
  // const { mutateAsync: makeBid, isLoading, error } = useMakeBid(bidContract);
  ...
  return (
    <Container color={'gray.50'} maxW={'1200px'} p={5} my={120}>
      <SimpleGrid columns={2} spacing={6}>
        <Stack spacing={'20px'}>
          <Box borderRadius={'6px'} overflow={'hidden'} boxShadow="outline">
        ...
        <Stack spacing={'20px'}>
          ...
          <Stack color={'blackAlpha.800'} backgroundColor={'#EEE'} p={2.5} borderRadius={'6px'}>
            <Text>Price:</Text>
            <Skeleton isLoaded={!loadingMarketplace && !loadingDirectListing}>
          ...
          <Skeleton isLoaded={!loadingMarketplace || !loadingDirectListing || !loadingAuction}>
            <Stack spacing={5}>
              <Web3Button
                ...
                isDisabled={
                  (!auctionListing || !auctionListing[0]) && (!directListing || !directListing[0])
                }
              >
                Buy at asking price
              </Web3Button>
